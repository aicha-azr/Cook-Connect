...
<div className="fixed top-0 flex flex-wrap w-full ">
  <div className="px-3 w-full bg-blanc shadow-bottom shadow-sm shadow-black-maron">
    ...
    <div className="relative flex items-center ml-2 lg:ml-4">
        <a href="/addPost" className="flex items-center justify-center w-12 h-12 text-base font-medium leading-normal text-center align-middle transition-colors duration-150 ease-in-out bg-transparent border border-solid shadow-none cursor-pointer rounded-2xl  text-black-maron border-black-maron hover:text-primary hover:bg-jaune active:text-primary focus:text-primary w-fit p-2">
            add post 
        </a>
    </div>
    </div>
 ...
 <div className={`fixed top-20 right-20 z-50 ${isOpen ? 'block' : 'hidden'} p-4 bg-bleu-ciel shadow-md rounded-md shadow-orange`} id="user-dropdown">
        <div className="px-4 py-3">
          <span className="block text-sm font-bold text-gray-900 dark:text-white">Bonnie Green</span>
          <span className="block text-sm text-orange truncate dark:text-gray-400">name@flowbite.com</span>
        </div>
        <ul className="py-2" aria-labelledby="user-menu-button">
          <li>
            <a href="#" className="block px-4 py-2 text-sm text-black-maron hover:text-orange border-t border-orange">Sign out</a>
          </li>
        </ul>
      </div>
